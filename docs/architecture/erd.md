...
- `qr_expires_at`'s offset from timeslot start, and the values behind `orders.status`/cancellation rules, are still governed by the `[TBD]` business rules in `docs/investment/requirements.md` (QR expiry rule, refund cutoff) — this ERD defines the *columns* those rules operate on, not the rules' concrete values.
